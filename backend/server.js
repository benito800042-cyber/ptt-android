...
if (!token) console.warn('PTT_SHARED_TOKEN is not set; all client connections will be rejected.');
...
function presence(){ broadcast({type:'presence', participants:[...clients.values()].map(publicProfile), speaker:speaker && publicProfile(speaker)}); }
const server=http.createServer((req,res)=>{
  const pathname = new URL(req.url || '/', 'http://localhost').pathname;
  if(pathname==='/healthz'){
    res.writeHead(200,{'content-type':'application/json; charset=utf-8','cache-control':'no-store'});
    return res.end(JSON.stringify({ok:true,clients:clients.size,speaker:!!speaker}));
  }
...
wss.on('connection',(ws,req)=>{
  ...
  ws.on('message', raw=>{
    ...
    if(!c){
      if(m.type!=='hello' || !token || m.token!==token || !m.profile || !m.profile.name || !m.profile.taxi || !m.profile.phone){ws.close(1008,'authentication/profile required');return;}
      ...
      send(c,{type:'welcome',id:c.id,participants:[...clients.values()].map(publicProfile),speaker:speaker&&publicProfile(speaker)}); presence(); return;
    }
